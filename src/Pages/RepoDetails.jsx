...
function RepoDetails() {
  ...
  useEffect(() => {
    fetch(`https://api.github.com/repos/gekwealor/${id}`)
  ...
  useEffect(() => {
    fetch(`https://api.github.com/repos/gekwealor/${id}/branches`)
    .then((response) => (response.json()))
    .then((data) => {
      setBranch(data)
    })
  ...
  useEffect(() => {
    fetch(`https://api.github.com/repos/gekwealor/${id}/deployments`)
    .then((response) => (response.json()))
    .then((data) => {
      setDeployment(data)
    })
    ...
    return (
        <div id="RepoDetail">
            <div className="RepoDetail-card">
                ...
                <p>Live site: {deployment.length === 0 ? `none` : <a href={`https://gekwealor.github.io/${details.name}`}>gekwealor.github.io/{details.name}</a>}</p>
